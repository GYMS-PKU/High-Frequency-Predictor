...
class DataProcessor:
    ...
    @staticmethod
    def tick_3s_multi(data_tick,window_past,window_future,method0,upper,lower):  # 基于n个文件200*20的矩阵，在矩阵上进行信息扩充，返回n个200*p的矩阵（其中p>=20）
        """
        ...
        for i in tqdm(range(len(data_tick))):
            ...
            data_1['mid-price'] = (data_1['askprice1'] + data_1['bidprice1']) / 2
            data_1['return_next'] = data_1['mid-price'].shift(-window_future) / data_1['mid-price'] - 1
            data_1 = data_1.dropna(axis=0, how='any')
            if method0 == 'classification':
                data_1['return_next'] = data_1['return_next'].apply(lambda x: 1*(x>upper)+(-1)*(x<lower))

            data_tick_y.extend(list(data_1['return_next'].values[window_past-1:]))
            del data_1['return_next']

            length1 = len(data_1)
            for j in range(length1 - window_past + 1):
                data_tick_x.append(data_1.iloc[j:j+window_past])
    ...
    def data_3s_feature_all(self,window_past,window_future,method0,upper,lower):
        ...
        data_tick_u_x, data_tick_u_y = function1(data_tick_u,window_past,window_future,method0,upper,lower)
        data_tick_d_x, data_tick_d_y = function1(data_tick_d,window_past,window_future,method0,upper,lower)
        data_tick_s_x, data_tick_s_y = function1(data_tick_s,window_past,window_future,method0,upper,lower)
        return data_tick_u_x, data_tick_u_y, data_tick_d_x, data_tick_d_y, data_tick_s_x, data_tick_s_y
    
    
    def data_feature_input_part(self,r,column_num_multi,column_num_single,data_tick_u_x,data_tick_u_y, \
                                data_tick_d_x, data_tick_d_y, data_tick_s_x, data_tick_s_y):
        # r为训练集所占数据比例
        ...
        y_train = np.hstack([data_tick_u_y[select_u], data_tick_s_y[select_s], data_tick_d_y[select_d]])

        x_test = np.vstack([data_tick_u_x[rest_u, :], data_tick_s_x[rest_s, :], data_tick_d_x[rest_d, :]])
        y_test = np.hstack([data_tick_u_y[rest_u], data_tick_s_y[rest_s], data_tick_d_y[rest_d]])
